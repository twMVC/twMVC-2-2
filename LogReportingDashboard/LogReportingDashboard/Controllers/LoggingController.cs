...
using LogReportingDashboard.Models.Google.Visualization;
using LogReportingDashboard.Models.Repository;
using LogReportingDashboard.Services.Paging;
using LogReportingDashboard.Services.RSS;
using LogReportingDashboard.ViewModels;
...
namespace LogReportingDashboard.Controllers
{
    ...
    public class LoggingController : Controller
    {
        ...
        public JsonResult ChartDataByTimePeriod(string Period, string LoggerProviderName, string logLevel, string keyFormatString, Func<LogEvent, Object> groupByClause)
        {
            ...
            var groupedByDateAndThenName = groupedByDate.Select(group => new 
            { 
                Key = group.Key, 
                NestedGroup = group.ToLookup(result => result.LoggerProviderName, result => result.Id) 
            });
            ...
            foreach (var myDate in groupedByDateAndThenName)
            {
                ...
                string dateString = (myDate.Key is DateTime) 
                    ? ((DateTime)myDate.Key).ToString(keyFormatString) 
                    : myDate.Key.ToString();
