            ...
            switch (logProviderName)
            {
                case "All":
                    foreach (string providerName in logProviders.Keys)
                    {
                        ...
                        if (logList.Count() > 0)
                        {
                            list = (list == null || list.Count.Equals(0))
                                ? logList.ToList() 
                                : list.Union(logList).ToList();
                        }
                    }
                    break;

                default:
                    list = GetProvider(logProviderName).GetByDateRangeAndType(pageIndex, pageSize, start, end, logLevel).ToList();
                    break;
            }

            if (list.Count.Equals(0))
            {
                return new PagedList<LogEvent>(new List<LogEvent>(), pageIndex, pageSize);
            }
            else
            {
                return new PagedList<LogEvent>(list.OrderByDescending(d => d.LogDate).ToList(), pageIndex, pageSize);
            }
        }
